...
module DatapathMultiCycle (
  ...
  logic [7:0] div_stall_sr;

  always_ff @(posedge clk) begin
    if      (rst)                             div_stall_sr <= 8'b0;
    else if (is_div && div_stall_sr == 8'b0)  div_stall_sr <= 8'hFF; // 8 ones = 7 stall cycles + retire
    else                                      div_stall_sr <= div_stall_sr >> 1;
  end

  wire div_stalling = is_div && (div_stall_sr != 8'b0000_0001);
