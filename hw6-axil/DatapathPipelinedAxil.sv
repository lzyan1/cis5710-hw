...
// NEW: state carried through the G stage (F sent the AR request; G awaits RDATA).
// No instruction bits yet — those arrive as imem.RDATA at the G→D boundary.
typedef struct packed {
  logic [`REG_SIZE] pc;
  cycle_status_e    cycle_status;
} stage_g_t;

// state before D  (G→D register; insn is imem.RDATA latched at end of G)
typedef struct packed {
  logic [`REG_SIZE] pc;
...
module DatapathPipelinedAxil (
    ...
    // interface to data memory/cache  (disconnected for Step 1)
  ...
  // =====================================================
  // STALL / FLUSH SIGNALS  (declared up front; driven later)
  // =====================================================

  // x_branch_taken is driven in the execute stage comb block below.
  // d_load_use_stall and d_div_stall are driven after the decode stage.
  // Declaring them here so the F-stage always_ff can reference them.
  logic             x_branch_taken;
  logic [`REG_SIZE] x_branch_target;

  wire d_load_use_stall;  // driven below by continuous assignment
  wire d_div_stall;       // driven below by continuous assignment

  // pipeline_stall: both F and G stall; X receives a bubble
  ...
  always_ff @(posedge clk) begin
    if (rst) begin
      ...
      if (x_branch_taken)
        ...
        f_pc_current <= f_pc_current;   // hold — do not re-request same PC
      else
        f_pc_current <= f_pc_current + 32'd4;
    end
  end

  // ── AXIL instruction-memory connections ──────────────────────────────────
  //
  // AR channel: F stage drives the address each cycle it advances.
  //   ARVALID=0 when stalling (would duplicate the in-flight request) or
  //   when branch is taken (current F PC is on the wrong path).
  assign imem.ARADDR  = f_pc_current;
  assign imem.ARVALID = !pipeline_stall && !x_branch_taken;
  assign imem.ARPROT  = 3'b000;
  //
  // R channel: lower RREADY when G cannot advance (pipeline stall), so the
  //   skid buffer holds RDATA for the extra cycle(s).
  //   When branch is taken we still accept (drain) the in-flight response
  //   even though we will discard it — G is being flushed to a bubble.
  assign imem.RREADY  = !pipeline_stall;
  //
  // Write channels: instruction port is read-only; tie off permanently.
  ...
  // No F-stage disasm: instruction bits do not exist in Fetch; they arrive
  // from imem.RDATA one cycle later (in G).

  // =====================================================
  // F→G PIPELINE REGISTER
  // =====================================================
  //
  // G holds the PC of the in-flight AR request.  The corresponding RDATA
  // (instruction bits) arrives from the memory at the end of this stage and
  // is latched into the G→D register below.
  ...
  always_ff @(posedge clk) begin
    if (rst) begin
      ...
      // Flush G: the in-flight response will be drained (RREADY=1) but
      // discarded.  Mark as taken-branch bubble.
      g_state <= '{pc: 0, cycle_status: CYCLE_TAKEN_BRANCH};
    end else if (pipeline_stall) begin
      g_state <= g_state;  // hold
    end else begin
      g_state <= '{
        pc:           f_pc_current,
  ...
  // G-stage disasm: shows the instruction word arriving from memory this cycle.
  wire [255:0] g_disasm;
  ...
  // =====================================================
  // G→D PIPELINE REGISTER  (DECODE)
  // =====================================================
  //
  // imem.RDATA is the instruction returned by the memory for the request
  // that was issued when this G-state entry was in the F stage.
  ...
  always_ff @(posedge clk) begin
    if (rst) begin
      ...
      decode_state <= '{
        pc:           g_state.pc,
        insn:         imem.RDATA,       // instruction bits arrive here
  ...
  assign d_load_use_stall =
    ...
    ((d_uses_rs1 && execute_state.rd == d_rs1) ||
     (d_uses_rs2 && execute_state.rd == d_rs2 && d_opcode != OpcodeStore));
  ...
  always_ff @(posedge clk) begin
    if (rst) begin
      ...
      // Branch taken: flush D→X (3 stages total: F, G, D all flushed)
      execute_state <= '{
    ...
    case (x_opcode)
      ...
      OpcodeRegImm: begin
        case (x_funct3)
          ...
          3'b010: x_rd_data = (x_s_rs1 <  x_s_imm_i) ? 32'd1 : 32'd0; // slti
          3'b011: x_rd_data = (x_rs1   <  execute_state.imm_i_sext) ? 32'd1 : 32'd0; // sltiu
          3'b100: x_rd_data = x_rs1 ^ execute_state.imm_i_sext; // xori
          3'b110: x_rd_data = x_rs1 | execute_state.imm_i_sext; // ori
          3'b111: x_rd_data = x_rs1 & execute_state.imm_i_sext; // andi
          3'b001: x_rd_data = x_rs1 << x_imm_shamt;              // slli
        ...
        if (x_funct7 == 7'd0) begin
          ...
          case (x_funct3)
            ...
            3'b100, 3'b101, 3'b110, 3'b111: begin
              x_rf_we = 1'b0; // result arrives via parallel div pipeline
            end
            default: x_rf_we = 1'b0;
          endcase
        end else begin
  ...
  always_ff @(posedge clk) begin
    if (rst) begin
      ...
      // Divider result arrives: write div output into M, bypassing normal X→M
      memory_state <= '{
        pc:           div_q[6].pc,
        insn:         div_q[6].insn,
      ...
      // Div just entered X: bubble this slot while it runs
      memory_state <= '{
        pc: 0, insn: 0, cycle_status: CYCLE_DIV,
        rd: 0, rd_data: 0, rf_we: 0, halt: 0,
        is_load: 0, is_store: 0, mem_funct3: 0, mem_addr: 0, store_data: 0
  ...
  // =====================================================
  //
  // Step 1: dmem is disconnected; loads return 0, stores are no-ops.
  // Local wires mirror the old single-cycle interface so the logic below
  // compiles unchanged and Step 2 only needs to wire these to dmem.

  logic [`REG_SIZE]  addr_to_dmem;
  logic [`REG_SIZE]  store_data_to_dmem;
  logic [3:0]        store_we_to_dmem;
  wire  [`REG_SIZE]  load_data_from_dmem = 32'd0; // disconnected for Step 1

  logic [7:0]  m_load_byte;
  logic [15:0] m_load_half;

  // WM bypass: forward W→rd to an in-flight store's rs2
  wire [4:0]        m_store_rs2 = memory_state.insn[24:20];
  ...
  // ── Stub out dmem for Step 1 ──────────────────────────────────────────────
  // (addr_to_dmem / store_data_to_dmem / store_we_to_dmem are computed above
  //  but not yet routed to the interface; that happens in Step 2.)
  assign dmem.ARADDR  = 32'd0;
  assign dmem.ARVALID = 1'b0;
  assign dmem.ARPROT  = 3'b000;
  assign dmem.RREADY  = 1'b1;
  assign dmem.AWADDR  = 32'd0;
  assign dmem.AWVALID = 1'b0;
  assign dmem.AWPROT  = 3'b000;
  assign dmem.WDATA   = 32'd0;
  assign dmem.WSTRB   = 4'b0000;
  assign dmem.WVALID  = 1'b0;
